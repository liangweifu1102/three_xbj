...
unsigned short XGearRatio,YGearRatio,ZGearRatio,UGearRatio;     //电子此轮比
...
void ReadRobotCurrent(void)
{
   ...
   SystemParam->XDistance = PluseToPosition(X_AXIS, MV_Get_Command_Pos(X_AXIS));  //读当前X轴位置
   SystemParam->YDistance = PluseToPosition(Y_AXIS, MV_Get_Command_Pos(Y_AXIS));  //读当前Y轴位置
   SystemParam->ZDistance = PluseToPosition(Z_AXIS, MV_Get_Command_Pos(Z_AXIS));  //读当前Z轴位置
//   SystemParam->UDistance = MV_Get_Command_Pos(U_AXIS) / UGearRatio;  //读当前U轴位置
